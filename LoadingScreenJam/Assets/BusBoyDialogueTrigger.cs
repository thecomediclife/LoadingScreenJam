...
public class BusBoyDialogueTrigger : MonoBehaviour {
	...
	void Start () {
		...
		if (PlayerPrefs.GetString ("BusTicket") == "") {
	...
	void BeginDialogue() {
		...
		if (PlayerPrefs.GetString (playerPrefVarName) == "True_1") {
			//Player has failed mission
			question = false;
			currentTextArray = response2Array;

		} else {
			if (PlayerPrefs.GetString("BusTicket") == "True") {
				//Player has failed mission and found ticket
				question = false;
				currentTextArray = successArray;
			} else {
				//Player has not failed mission, and not found ticket yet.
				question = true;
				currentTextArray = firstDialogueArray;
			}
		}
	...
	void EndDialogue() {
		...
		if (PlayerPrefs.GetString ("BusTicket") == "True" && currentTextArray != response2Array) {
